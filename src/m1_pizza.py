...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(100, 150),
                rg.Point(50, 200),
                rg.Point(0, 150),
                rg.Point(50, 100)]
    circle = rg.Circle(rg.Point(50, 150), 50)
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  11 green dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 200), 20)
    draw_points_on_circle(window, circle, 11, 'green')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  10 slices, thick (thickness=10) orange lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 175)
    circle.outline_thickness = 2
    pizza(window, circle, 10, 'orange', 10)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    point_list = generate_points_on_circle(circle, number_of_slices)
    for i in point_list:
        line = rg.Line(circle.center, i)
        line.color = color
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  9 segments with thin red lines'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 9, 'red', 1)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for i in range(number_of_segments):
        line = rg.Line(points_list[i - 1], points_list[i])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON tests 4:  9 green lines, hops = 4'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    fancy_polygon(window, circle, 9, 4, 'green', 3)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for i in range(number_of_lines):
        line = rg.Line(points_list[i], points_list[i - hops_to_next_point])
        line.color = color
        line.thickness = thickness
        line.arrow = "last"
        line.attach_to(window)
